...
<%
    ...
    try (Connection con = ConexionDB.getConnection()) {
        String sql = "SELECT t.cliente_id, t.propiedad_id, t.tipo, t.fecha, t.monto " +
        ...
        if (rs.next()) {
            transaccion.put("cliente_id", rs.getInt("cliente_id"));
            transaccion.put("propiedad_id", rs.getInt("propiedad_id"));
            transaccion.put("tipo", rs.getString("tipo"));
            transaccion.put("fecha", rs.getDate("fecha"));
            transaccion.put("monto", rs.getBigDecimal("monto"));
        } else {
            rs.close();
            ps.close();
            response.sendRedirect("transacciones.jsp");
    ...
    try (Connection con = ConexionDB.getConnection()) {
        String sql = "SELECT id_usuario, nombre FROM usuarios WHERE rol_id=2";
        PreparedStatement ps = con.prepareStatement(sql);
        ResultSet rs = ps.executeQuery();
        while (rs.next()) {
    ...
    // Listado de propiedades de la inmobiliaria
...
<title>Editar Transacción - InmoConnect</title>
...
<div class="main-content">
    ...
    <div class="card shadow">
        <div class="card-header bg-primary text-white">
            <h5 class="mb-0"><i class="fas fa-edit me-2"></i>Modificar Transacción</h5>
        </div>
        <div class="card-body">
            <form action="procesarEditarTransaccion.jsp" method="post" class="needs-validation" novalidate>
                <input type="hidden" name="id_transaccion" value="<%= idTransaccion %>">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="cliente" name="cliente_id" required>
                        ...
                        <% for (Map<String,Object> c : clientes) { 
                               int cid = (int)c.get("id_usuario");
                               String selected = cid == (int)transaccion.get("cliente_id") ? "selected" : "";
                        %>
                            <option value="<%= cid %>" <%= selected %>><%= c.get("nombre") %></option>
                        <% } %>
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="propiedad" name="propiedad_id" required>
                        ...
                        <% for (Map<String,Object> p : propiedades) { 
                               int pid = (int)p.get("id_propiedad");
                               String selected = pid == (int)transaccion.get("propiedad_id") ? "selected" : "";
                        %>
                            <option value="<%= pid %>" <%= selected %>><%= p.get("titulo") %></option>
                        <% } %>
                    </select>
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <select class="form-select" id="tipo" name="tipo" required>
                            <option value="venta" <%= "venta".equals(transaccion.get("tipo")) ? "selected" : "" %>>Venta</option>
                            <option value="alquiler" <%= "alquiler".equals(transaccion.get("tipo")) ? "selected" : "" %>>Alquiler</option>
                        </select>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="date" class="form-control" id="fecha" name="fecha" 
                               value="<%= transaccion.get("fecha") %>" required>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" step="0.01" class="form-control" id="monto" name="monto" 
                               value="<%= transaccion.get("monto") %>" required>
